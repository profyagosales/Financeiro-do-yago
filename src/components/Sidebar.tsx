...
const sections: Section[] = [
  ...
  {
    label: "Sistema",
    items: [{ type: "item", label: "Configurações", to: "/configuracoes", icon: Settings }],
  },
];
    ...
    <RTooltip.Provider delayDuration={200} skipDelayDuration={200}>
      ...
      >
        <div className="flex h-full flex-col">
          <div className="m-2 flex items-center rounded-2xl sidebar-header p-4 ring-1 ring-white/5">
            <Logo size="lg" />
          ...
          <nav ref={navRef} className="flex-1 overflow-y-auto px-3 py-4 scrollbar-none">
...
function NavLeafLink({ leaf, collapsed }: { leaf: NavLeaf; collapsed?: boolean }) {
  ...
  const LinkEl = (
    ...
    >
      {Icon ? (
        <Icon className="h-4 w-4 text-slate-400 group-[.sb-active]:text-emerald-300 group-hover:text-white" />
      ) : null}
