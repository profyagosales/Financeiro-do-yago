...
import { CandlestickChart, MoreHorizontal, Pencil, Plus, Trash2 } from "lucide-react";
...
export default function CarteiraBolsa() {
  const { rows, loading, kpis, add, update, remove } = useInvestments({ type: "Ações" });
  ...
  const onUpdate = async (id: number, patch: any) => {
    try {
      await update(id, { ...patch, type: "Ações" });
