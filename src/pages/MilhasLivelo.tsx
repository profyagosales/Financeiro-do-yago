...
export default function MilhasLivelo({ program = "livelo" }: { program?: Program }) {
  ...
  const [movs, setMovs] = useState<MilesMovement[]>([
    {
      ...
      points: 1200,
      partner: "Compra",
      expires_at: dayjs().add(15, "day").format("YYYY-MM-DD"),
    },
    {
      id: "2",
    ...
    {
      id: "3",
      date: dayjs().subtract(1, "month").date(20).format("YYYY-MM-DD"),
      kind: "earn",
      points: 3000,
      partner: "Cartão",
    },
    ...
    return movs
      .filter(
        (m) =>
          m.kind === "earn" &&
          m.expires_at &&
          dayjs(m.expires_at).isAfter(now) &&
          dayjs(m.expires_at).isBefore(limit)
      )
      .reduce((s, m) => s + m.points, 0);
  }, [movs]);
  ...
  const ganhosMes = useMemo(
    () =>
      movs
        .filter(
          (m) => m.kind === "earn" && m.date.startsWith(thisMonth)
        )
        .reduce((s, m) => s + m.points, 0),
    [movs, thisMonth]
  );
  const resgMes = useMemo(
    () =>
      movs
        .filter(
          (m) => m.kind === "redeem" && m.date.startsWith(thisMonth)
        )
        .reduce((s, m) => s + m.points, 0),
    [movs, thisMonth]
  );
