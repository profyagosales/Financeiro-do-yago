...
export default function CarteiraRendaFixa() {
  // hook já normaliza tipo e agrega totals
  const { rows, loading, kpis, add, update, remove } = useInvestments({ type: "Renda fixa" });
  ...
  const onCreate = async (payload: any) => {
    try {
      await add({ ...payload, type: "Renda fixa" });
  ...
  const onUpdate = async (id: number, patch: any) => {
    try {
      await update(id, { ...patch, type: "Renda fixa" });
  ...
  breadcrumbs={[{ label: "Investimentos", href: "/investimentos/renda-fixa" }, { label: "Carteira", href: "/investimentos/renda-fixa" }, { label: "Renda Fixa" }]}
