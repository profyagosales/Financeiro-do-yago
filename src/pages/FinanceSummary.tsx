/* Migrated from FinancasResumo.tsx: modernizado com hero + KPI grid */
import {
    ArrowDownCircle,
    ArrowUpCircle,
    CalendarClock,
...
type SourceValue = any; // substituir quando Picker real disponível
const SourcePicker = (props: any) => { void props; return null; };
const CategoryPicker = (props: any) => { void props; return null; };
const InsightBar = (props: any) => { void props; return null; };
...
const ForecastMiniChart = (props: any) => { void props; return null; };
const formatCurrency = (v: any) => String(v ?? '');
const RecurrenceList = (props: any) => { void props; return null; };
const AlertList = (props: any) => { void props; return null; };
...
const YAxis = () => null;
const RechartsTooltip = (props: any) => { void props; return null; };
const Legend = () => null;
...
export default function FinanceSummary() {
  ...
  const recentTransactions = useMemo(() => filtered.slice(-8).reverse(), [filtered]);
  ...
  return (
    <SectionChroming clr="financas" decorate className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
        ...
        <CategoryPicker value={categoryId} onChange={setCategoryId} className="w-full sm:w-52" allowClear />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-12">
        <WidgetCard className="u-card-base lg:col-span-6">
          ...
          {transacoes.length > 0 ? (
            <>
              ...
              <p className="mt-2 text-sm text-zinc-500">Saldo no fim do mês: {formatCurrency(forecastBalance)}</p>
            </>
          ) : (
        ...
        <WidgetCard className="u-card-base lg:col-span-6">
          ...
          {transLoading ? (
            ...
            <EmptyState title="Sem transações" message="Adicione uma transação para ver o fluxo de caixa." action={<Button size="sm" onClick={() => setModalOpen(true)}>Nova transação</Button>} />
          )}
          <WidgetFooterAction to="/financas/mensal">Ver detalhes</WidgetFooterAction>
        </WidgetCard>

  <WidgetCard className="glass bg-gradient-to-br from-white/60 to-white/30 lg:col-span-6">
          ...
          {transLoading ? (
            ...
            <EmptyState title="Sem dados" message="Adicione transações para visualizar." action={<Button size="sm" onClick={() => setModalOpen(true)}>Nova transação</Button>} />
          )}
          <WidgetFooterAction to="/financas/anual">Ver detalhes</WidgetFooterAction>
        </WidgetCard>

  <WidgetCard className="glass bg-gradient-to-br from-white/60 to-white/30 lg:col-span-4">
          ...
          {transLoading ? (
            ...
            <EmptyState title="Sem dados" message="Adicione transações para ver categorias." action={<Button size="sm" onClick={() => setModalOpen(true)}>Nova transação</Button>} />
          )}
          <WidgetFooterAction to="/financas/mensal">Ver detalhes</WidgetFooterAction>
        </WidgetCard>

  <WidgetCard className="glass bg-gradient-to-br from-white/60 to-white/30">
          <WidgetHeader title="Contas a vencer" />
          {upcomingBills.length > 0 ? (
            <AlertList items={upcomingBills} />
          ) : (
            <EmptyState title="Nenhuma conta a vencer" action={<Button size="sm" onClick={() => setModalOpen(true)}>Nova transação</Button>} />
          )}
          <WidgetFooterAction to="/financas/mensal">Ver detalhes</WidgetFooterAction>
        </WidgetCard>
        ...
        <WidgetCard className="u-card-base lg:col-span-4">
          ...
          {wishlistImpact > 0 ? (
            <p className="px-4 py-6 text-3xl font-semibold">{formatCurrency(wishlistImpact)}</p>
          ) : (
            <EmptyState title="Sem desejos comprados" />
          )}
        </WidgetCard>

        <RecurrenceList className="u-card-base lg:col-span-4" items={recurrences.map((r) => ({ name: r.description, amount: r.amount }))} />
            ...
            <div className="overflow-x-auto">
              <ul className="flex gap-4">
                {uiTransacoes.slice(0, 5).map(t => (
                  <li key={t.id} className="min-w-[14rem] rounded-md border p-3 text-sm">
                    <div className="mb-1 flex items-center gap-2">
                      <span className="truncate">{t.description}</span>
                      {t.origin?.wishlist_item_id && <Badge variant="outline">Origem: Desejo</Badge>}
                    </div>
  ...
  <div className="col-span-12 rounded-2xl shadow/soft bg-background/60 backdrop-blur border border-white/10 p-4">
          <Heading level={3}>Transações recentes</Heading>
          {transLoading ? (
            <SkeletonLine className="h-40" />
          ) : recentTransactions.length ? (
            <TransactionsTable transacoes={recentTransactions} onEdit={() => {}} onDelete={async () => {}} />
          ) : (
